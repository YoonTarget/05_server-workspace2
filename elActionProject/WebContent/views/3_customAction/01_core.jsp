...
<body>
	...
	* 변수 선언과 동시에 초기화 (c:set var="" value="" [scope=""])
	...
	시작태그와 종료태그 사이에 값을 지정한 result : ${ requestScope.result } <br>
	...
	* 변수(데이터) 출력 (c:out value="출력하고자 하는 값" [default="기본값"] [escapeXml="true|false"])
	...
	- 각각의 조건들을 c:choose의 하위요소로 c:when을 통해서 작성 (else문 역할 == c:otherwise)
	</pre>
	...
	<c:choose>
		...
		<!-- 
			c:choose와 c:when 사이에는 주석을 걸면 안된다 => 오류 남
			주석은 c:when 안에 작성!!
		-->
			<!-- num1이 20보다 큰 경우 -->
			<b>처음 뵙겠습니다.</b>
	...
	<% 
		...
		list.add(new Person("김봉석", 19, "남자"));
		list.add(new Person("장희수", 19, "여자"));
		list.add(new Person("이강훈", 19, "남자"));
	%>
