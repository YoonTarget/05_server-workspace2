...
public class JqAjaxController3 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Member m = new Member(1, "박철수", 30, "남"); // 조회된 데이터가 다음과 같다는 가정
		
		// response.getWriter().print(m/*.toString()*/); // vo객체를 곧바로 응답시 .toString()을 바로 호출해서 의도한 결과를 얻지 못함
		...
		*/
		...
		response.setContentType("application/json; charset=UTF-8");
		//Gson gson = new Gson(); // Gson 객체.toJson(응답할 자바객체, 응답할 스트림);
