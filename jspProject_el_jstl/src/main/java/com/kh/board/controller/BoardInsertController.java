...
public class BoardInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			 ...
			 * 
			 * 		아래의 구문 한 줄의 실행만으로 넘어온 첨부파일이 해당 폴더에 무조건 업로드 됨!!
			 * 		
			 * 		MultipartRequest multiRequest = new MultipartRequest(request, 저장시킬폴더의경로, 용량제한, 인코딩값, 파일명수정시켜주는객체);
			 * 
			 * 		단, 업로드시 파일명은 수정해주는 게 일반적임!! 그래서 파일명 수정시켜주는 객체 제시해야됨
			 * 		=> 같은 파일명이 존재할 경우 덮어씌워질 수 있고, 파일명에 한글/특문/띄어쓰기가 포함될 경우 서버에 따라 문제가 발생할 여지가 있다.
			 * 
			 * 		기본적으로 파일명이 안 겹치도록 수정 작업해주는 객체가 있음
			 ...
			 * 		   }
			 * 
			 * 		나만의 방식대로 절대 안 겹치도록 rename 할 수 있게 나만의 FileRenamePolicy 클래스 만들기!!
			 ...
			 */
			 ...
			 // > Board : 카테고리 번호, 제목, 내용, 작성자 회원번호 뽑아서 BOARD INSERT
			 
			 // > Attachment : 넘어온 첨부파일이 있으면 원본명, 수정명, 저장경로 ATTACHMENT INSERT
			 ...
			 if(result > 0) {
				 ...
				 // 실패 => 에러페이지
				 request.setAttribute("errorMsg", "게시물 등록에 실패했습니다.");
